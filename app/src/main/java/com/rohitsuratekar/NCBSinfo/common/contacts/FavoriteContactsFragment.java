...
public class FavoriteContactsFragment extends Fragment implements SearchView.OnQueryTextListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        doSearch(rootView2);
        Boolean Firstvalue = PreferenceManager.getDefaultSharedPreferences(getContext()).getBoolean(Contacts.FIRST_TIME_CONTACT, true);

        if (Firstvalue){
            Database db = new Database(getContext());
            String[][] clist = new ContactList().allContacts();
            for (int i=0; i <clist.length; i++){
                new ContactsData(getContext()).add(new ContactModel(1, clist[i][0], clist[i][1],clist[i][2], clist[i][3], "0"));
            }
            PreferenceManager.getDefaultSharedPreferences(getContext()).edit().putBoolean(Contacts.FIRST_TIME_CONTACT, false).apply();
            db.close();
        }
